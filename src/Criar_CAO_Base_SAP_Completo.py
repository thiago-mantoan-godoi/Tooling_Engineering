...
def Comp_SAP_vs_CAO(
    ...
    for led in leadset_check:
        ...
        for ckt in tqdm(leadset_SAP, desc=f"Processando 1 [{led}]", colour="blue"):
        ...
        for i in tqdm(new_df_cao.index, desc=f"Processando 2 [{led}]", colour="blue"):
...
def comparar_fam_sap_cao_completo():
    os.system("cls")
    ...
    salvar_dict_df_em_excel(
        dict_consolidado, f"{caminho_output}\Análise_CAO_Completo_{data_atual}.xlsx"
    )

    df_new_cao.to_csv(
        f"{caminho_output}\Cadastro_CAO_{data_atual}.csv", sep=";", index=False
    )
    df_new_cao.dropna(how="all", axis=1).to_excel(
        f"{caminho_output}\Cadastro_CAO_{data_atual}.xlsx", index=False
    )
