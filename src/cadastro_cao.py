...
def gerar_ordens(path_cadastro, df, number=3):
    if number > 0:
...
def cadastro_componentes(dados):
    ...
    book_cabos = pd.read_excel(consultar_arquivos_base("Lista_de_cabos"),dtype=str)

    book_cabos = convert_legacy(book_cabos)

    dict_legacy = {
        row["Part Number"]: row["Legacy"] for i, row in book_cabos.iterrows()
    }
    ...
    # CARREGAMENTO 2
    book_term = pd.read_excel(consultar_arquivos_base("terminais_sem"),dtype=str)
    ...
    for i, row in book_term.iterrows():
        ...
        if "FASTON" in classification or "FASTON" in description:
            if "SLEEVE" in classification or "SLEEVE" in description:
...
def gerar_CrimpSTD(dados):
    df_path = consultar_arquivos_base(id_name="lista_app")
    with open(df_path, "rb") as f:
        df_spec = pd.read_excel(f, sheet_name="Crimpagem Access",dtype=str)[
            [
                "Terminal",
                "Bitola",
                "Isolação",
    ...
    for i in tqdm(df_completo.index, "Processando", colour="blue"):
        ...
        if pd.isna(selo) or selo == "" or selo == "nan":
            selo = 0
        ...
        for j in df_spec.index:
            ...
            if pd.isna(selo_spec) or selo_spec == "" or selo_spec == "nan":
                selo_spec = 0
...
def cadastro():
    ...
    df_completo["1. Cadastro Conferência"] = df_cadastro.dropna(how="all", axis=1)
